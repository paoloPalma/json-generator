...
export default function DynamicForm({ category, schema, onChange, formData }) {
  ...
  const getNextId = (items, idField) => {
    ...
    const maxId = items.reduce((max, item) => {
      ...
      return itemId && !isNaN(Number(itemId))
        ? Math.max(max, Number(itemId))
        : max;
    }, 0);
  ...
  const handleComplexImageUpload = async (
    stepIndex,
    field,
    itemIndex,
    subField,
  ...
  return (
    <div className="space-y-6">
        ...
        {!schema.additionalFields && (
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              <Input
                ...
                onChange={(e) => {
                  if (e.target.files && e.target.files[0]) {
                    compressAndConvertToBase64(e.target.files[0]).then(
                      (base64String) => {
              ...
              />
              {formData.FirstImage &&
                formData.FirstImage.startsWith("data:image") && (
      ...
      {/* Render additionalFields if they exist */}
      {schema.additionalFields && (
        <div className="space-y-4 mb-6 p-4 border rounded-md">
          <h3 className="text-lg font-medium">Informazioni Generali</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {Object.keys(schema.additionalFields).map((field) => {
              const fieldSchema = schema.additionalFields[field];

              return (
                <div key={field} className="space-y-2">
                  <Label htmlFor={field}>{fieldSchema.label}</Label>
                  {fieldSchema.type === "text" ? (
                    ...
                    <div className="space-y-2">
                      ...
                      />
                      {formData[field] &&
                        formData[field].startsWith("data:image") && (
                          <div className="mt-2 border rounded-md p-2">
                            <p className="text-sm text-gray-500 mb-2">
                              Anteprima:
                            </p>
                            <img
                              src={formData[field] || "/placeholder.svg"}
                              alt={`Anteprima ${fieldSchema.label}`}
                              className="max-h-40 max-w-full object-contain"
                            />
                          </div>
                        )}
                        ...
                        return (
                          <div key={field} className="space-y-4">
                            ...
                            {Array.isArray(step[field]) &&
                              step[field].map((item, itemIndex) => (
                                ...
                                >
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {Object.keys(fieldSchema.fields).map(
                                      (subField) => {
                                        const subFieldSchema =
                                          fieldSchema.fields[subField];
                            ...
                            >
                              ...
                              {fieldSchema.itemLabel || "elemento"}
                            </Button>
                          </div>
                        );
                      }
                          ...
                          {fieldSchema.type === "text" ? (
                            fieldSchema.label
                              ...
                              <div className="space-y-2">
                                ...
                                />
                                {step[field] &&
                                  step[field].startsWith("data:image") && (
                                    <div className="mt-2 border rounded-md p-2">
                                      <p className="text-sm text-gray-500 mb-2">
                                        Anteprima:
                                      </p>
